...
def _load_recovered_app() -> dict:
    ...
    from engine.file_manager import load_file_config, match_file_by_prefix, save_uploaded_file
    from engine.report_config import load_report_config

    access_control_path = BASE_DIR / "config" / "access_control.example.json"
    ...
    def load_access_control() -> dict:
        if access_control_path.exists():
            return _json.loads(access_control_path.read_text(encoding="utf-8"))
        return {"users": []}

    def save_access_control(access_control: dict) -> None:
        access_control_path.parent.mkdir(parents=True, exist_ok=True)
        access_control_path.write_text(_json.dumps(access_control, ensure_ascii=False, indent=2), encoding="utf-8")

    def user_auth_token(user: dict) -> str:
        payload = f"{user.get('username', '')}:{user.get('role', '')}"
        return hashlib.sha256(payload.encode("utf-8")).hexdigest()

    def authenticate_user(access_control: dict, username: str, password: str) -> dict | None:
        for user in access_control.get("users", []):
    ...
    def current_user(access_control: dict) -> dict | None:
        username = _st.session_state.get("authenticated_username")
        if not username:
            return None
        for user in access_control.get("users", []):
    ...
    def is_authenticated(access_control: dict) -> bool:
        return current_user(access_control) is not None

    def user_role_keys(user: dict | None) -> list[str]:
        if not user:
            return []
        role = str(user.get("role", "")).strip()
        return [role] if role else []

    def find_uploaded_file(prefix: str):
        return match_file_by_prefix(upload_dir, prefix)

    def _format_excel_cell_value(value):
        return "" if value is None else str(value)

    def _worksheet_to_html(*args, **kwargs):
        return "<div>当前版本未加载恢复字节码，工作表预览暂不可用。</div>"
    ...
    def render_top_user_bar(*args, **kwargs) -> None:
        return None

    def render_unified_navigation(*args, **kwargs) -> None:
        return None

    def render_base_file_uploaders(*args, **kwargs) -> None:
        _st.info("当前为降级启动模式。基础文件上传功能待恢复字节码后启用。")

    def render_permission_assignment_page(*args, **kwargs) -> None:
        _st.info("当前为降级启动模式。权限分配功能待恢复字节码后启用。")

    def render_published_workbook_view(*args, **kwargs) -> None:
        _st.info("当前为降级启动模式。已生成工作簿预览功能待恢复字节码后启用。")

    def build_report_dataset(*args, **kwargs):
        return _pd.DataFrame()

    def build_pdf_metric_values_df(*args, **kwargs):
        return _pd.DataFrame()

    def sync_remembered_login_token(*args, **kwargs) -> None:
        return None

    def render_workflow_home(*args, **kwargs) -> None:
        _st.subheader("财报智控平台")
        _st.warning("当前未加载恢复字节码文件，已进入降级启动模式。登录后可先确认环境和数据目录可用。")

    def main() -> None:
        _st.set_page_config(page_title="财报智控平台", layout="wide")
        apply_global_styles()
        access_control = load_access_control()
        if not is_authenticated(access_control):
            render_login_page(access_control)
            return
        render_top_user_bar(access_control)
    ...
    namespace.update(
        {
            ...
            "build_pdf_metric_values_df": build_pdf_metric_values_df,
            "build_report_dataset": build_report_dataset,
            "current_user": current_user,
            "find_uploaded_file": find_uploaded_file,
            "handle_login_submit": None,
            "is_authenticated": is_authenticated,
            "load_access_control": load_access_control,
            "main": main,
            "render_base_file_uploaders": render_base_file_uploaders,
            "render_login_page": None,
            "render_permission_assignment_page": render_permission_assignment_page,
            "render_product_header": render_product_header,
            "render_published_workbook_view": render_published_workbook_view,
            "render_top_user_bar": render_top_user_bar,
            "render_unified_navigation": render_unified_navigation,
            "render_workflow_home": render_workflow_home,
            "save_access_control": save_access_control,
            "sync_remembered_login_token": sync_remembered_login_token,
            "user_auth_token": user_auth_token,
            "user_role_keys": user_role_keys,
            "_format_excel_cell_value": _format_excel_cell_value,
            "_worksheet_to_html": _worksheet_to_html,
        }
    )
    return namespace
